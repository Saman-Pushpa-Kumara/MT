...
Modded: Professional UI, Auto Muxing, Auto-Dismiss Indicator & Clear Data
*/

window.ytproSabrDownload = async function() {
    // Force remove any old instance to guarantee the new UI renders
    document.getElementById("outerdownytprodiv")?.remove();
    document.getElementById("ytpro-custom-dl-style")?.remove();

    var ytproDownDiv = getDownloadElement();

    // Responsive Dark/Light Detection
    const isDark = document.cookie.indexOf("f6=40000") > -1 || document.documentElement.hasAttribute("dark") || window.matchMedia("(prefers-color-scheme: dark)").matches;
    const themeColor = isDark ? "#ffffff" : "#111111";

    // Fast Skeleton Loader
    ytproDownDiv.querySelector("#videoViewDiv").innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:50px 0;gap:12px;">
            <div style="width:34px;height:34px;border:3px solid ${isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.08)"};border-top-color:#0a84ff;border-radius:50%;animation:ytSpin 0.7s linear infinite;"></div>
            <span style="font-size:13.5px;font-weight:600;opacity:0.7;">Loading video streams...</span>
        </div>
    `;

    // 1. Video ID Retrieval
    var videoId = "";
    ...
    // 2. High Speed Dynamic Imports with Cache
    ...
    ytproDownDiv.querySelector("#videoViewDiv").innerHTML = `
        <div class="yt-dl-card" style="display:flex;align-items:center;justify-content:space-between;padding:8px 14px;margin-bottom:12px;">
            <span style="font-size:13.5px;font-weight:600;">Audio Track</span>
            <select id="selectLang" style="height:32px;border-radius:10px;border:0;padding:0 10px;font-size:13px;font-weight:600;color:${themeColor};background:${isDark ? "#2c2c2e" : "#e5e5ea"};outline:none;cursor:pointer;"></select>
        </div>
    `;
    ...
    // ── 1. Video with Sound (Master Unified Formats) ──
    function updateMuxFormats(langId = uniqueLanguages.find(arr => arr.isDefault)?.id || uniqueLanguages[0]?.id) {
        muxedDiv.innerHTML = `
            <div class="yt-dl-section-title">Video with Audio (Unified Single File)</div>
            <div class="yt-dl-card"></div>
        `;
            ...
            item.innerHTML = `
                <div style="display:flex;flex-direction:column;align-items:flex-start;text-align:left;">
                    <div style="display:flex;align-items:center;gap:6px;">
                        ...
                        <span class="yt-dl-tag">SOUND INCLUDED</span>
                        <span style="font-size:10px;font-weight:700;padding:2px 6px;border-radius:6px;background:${isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)"};">${mux.container.toUpperCase()}</span>
                    </div>
                    <span class="yt-badge-sub">Single Video File • ${mux.totalSizeFormatted}</span>
                </div>
                <div class="yt-dl-btn">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="${themeColor}" viewBox="0 0 16 16"><path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"/><path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"/></svg>
                </div>
            `;
    ...
    function updateAudioOnlyFormats(langId = uniqueLanguages.find(arr => arr.isDefault)?.id || uniqueLanguages[0]?.id) {
        ...
        header.innerHTML = `
            ...
            <svg style="transition:transform 0.25s ease;" xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="${isDark ? "#8e8e93" : "#86868b"}" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z"/></svg>
        `;
        ...
        audioOnly.forEach(aud => {
            ...
            item.innerHTML = `
                <div style="display:flex;flex-direction:column;align-items:flex-start;text-align:left;">
                    <span class="yt-badge-res">${aud.audioQuality ? aud.audioQuality.replace("AUDIO_QUALITY_", "") : "Audio"}</span>
                    <span class="yt-badge-sub">${aud.container.toUpperCase()} • ${aud.sizeFormatted}</span>
                </div>
                <div class="yt-dl-btn">
                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="${themeColor}" viewBox="0 0 16 16"><path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"/><path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"/></svg>
                </div>
            `;
    ...
    function updateThumbnails() {
        ...
        thumbs.forEach(thumb => {
            ...
            card.style.cssText = `background:${isDark ? "#212124" : "#f6f6f9"};border-radius:18px;padding:8px;cursor:pointer;border:1px solid ${isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)"};text-align:center;`;
            card.innerHTML = `
    ...
    function updateCaptions() {
        ...
        div.innerHTML = `<div class="yt-dl-card"></div>`;
        ...
        captions.forEach(cap => {
            ...
            row.style.cssText = `display:flex;align-items:center;justify-content:space-between;padding:12px 4px;border-bottom:1px solid ${isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)"};`;
            row.innerHTML = `
                <span style="font-size:13.5px;font-weight:600;">${cap?.name?.text}</span>
                <div style="display:flex;gap:6px;">
                    <span data-url="${cap.baseUrl}&fmt=srt" data-title="${t}" data-ext=".srt" style="padding:4px 8px;font-size:11px;font-weight:700;background:${isDark ? "#2c2c2e" : "#e5e5ea"};border-radius:8px;cursor:pointer;">SRT</span>
                    <span data-url="${cap.baseUrl}&fmt=vtt" data-title="${t}" data-ext=".vtt" style="padding:4px 8px;font-size:11px;font-weight:700;background:${isDark ? "#2c2c2e" : "#e5e5ea"};border-radius:8px;cursor:pointer;">VTT</span>
                    <span data-url="${cap.baseUrl}&fmt=sbv" data-title="${t}" data-ext=".txt" style="padding:4px 8px;font-size:11px;font-weight:700;background:${isDark ? "#2c2c2e" : "#e5e5ea"};border-radius:8px;cursor:pointer;">TXT</span>
                </div>
            `;
    ...
    async function downloadSABRStream(videoItag, audioItag, isWebm, langId, enabledTrack) {
        ...
        function createProgressRow(streamTitle) {
            ...
            elProgressBar.style.cssText = `display:flex;width:100%;height:14px;background:${isDark ? "#2c2c2e" : "#e5e5ea"};border-radius:20px;overflow:hidden;`;
        ...
        downloaderDiv.innerHTML = `
            <div style="font-size:14.5px;font-weight:700;margin-bottom:2px;color:${isDark ? "#fff" : "#000"};display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:1;overflow:hidden;">${safeTitle}</div>
            <div style="font-size:11.5px;opacity:0.6;margin-bottom:14px;">High-speed streaming in progress...</div>
        `;

        // 1. Video Only Download
        ...
        // 2. Audio Only Download
            ...
            var fileName = `${safeTitle}_audio_${new Date().getTime()}.${containerExt == 'mp4' ? 'm4a' : 'opus'}`;
        ...
        // 3. Combined Video + Audio (Single Unified File with Auto-Muxing)
            ...
            window.Android?.showToast?.('Combining Video and Sound into Single File...');
...
// ── Auto Remove & Dismiss Indicator / Progress Logo ──
function finishAndDismissDownload() {
    window.Android?.showToast?.("Download Complete!");
    setTimeout(() => {
        const ind = document.getElementById("ytproDownloadIndicator");
        const status = document.getElementById("ytProDownloaderDiv");
        if (ind) {
            ind.style.transition = "all 0.4s ease";
            ind.style.opacity = "0";
            ind.style.transform = "scale(0.5)";
...
// ── Build Isolated Modern UI ──
function getDownloadElement() {
    const isDark = document.cookie.indexOf("f6=40000") > -1 || document.documentElement.hasAttribute("dark") || window.matchMedia("(prefers-color-scheme: dark)").matches;
    const themeColor = isDark ? "#ffffff" : "#111111";

    const styleEl = document.createElement("style");
    styleEl.id = "ytpro-custom-dl-style";
    styleEl.innerHTML = `
        @keyframes ytSpin { to { transform: rotate(360deg); } }
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
        @keyframes slideUp { from { transform: translateY(100%); } to { transform: translateY(0); } }
        #downytprodiv * { box-sizing: border-box !important; font-family: -apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Segoe UI', Roboto, sans-serif !important; }
        #downytprodiv::-webkit-scrollbar { display: none !important; }
        .yt-dl-section-title { font-size: 11px !important; font-weight: 700 !important; text-transform: uppercase !important; letter-spacing: 0.8px !important; color: ${isDark ? "#8e8e93" : "#86868b"} !important; margin: 14px 0 8px 4px !important; text-align: left !important; }
        .yt-dl-card { background: ${isDark ? "#212124" : "#f6f6f9"} !important; border-radius: 20px !important; padding: 6px 12px !important; margin-bottom: 12px !important; border: 1px solid ${isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)"} !important; }
        .yt-dl-item { display: flex !important; align-items: center !important; justify-content: space-between !important; padding: 12px 4px !important; min-height: 50px !important; cursor: pointer !important; text-decoration: none !important; transition: opacity 0.15s ease !important; }
        .yt-dl-item:not(:last-child) { border-bottom: 1px solid ${isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)"} !important; }
        .yt-dl-item:active { opacity: 0.65 !important; }
        .yt-badge-res { font-size: 14.5px !important; font-weight: 700 !important; color: ${isDark ? "#fff" : "#111"} !important; }
        .yt-badge-sub { font-size: 11.5px !important; font-weight: 500 !important; opacity: 0.65 !important; margin-top: 2px !important; }
        .yt-dl-btn { height: 34px !important; width: 34px !important; border-radius: 50% !important; background: ${isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.05)"} !important; display: flex !important; align-items: center !important; justify-content: center !important; flex-shrink: 0 !important; }
        .yt-dl-tag { font-size: 10px !important; font-weight: 700 !important; padding: 2px 7px !important; border-radius: 6px !important; background: rgba(52,199,89,0.15) !important; color: #34c759 !important; text-transform: uppercase !important; }
        .yt-dl-accordion-btn { display: flex !important; align-items: center !important; justify-content: space-between !important; width: 100% !important; padding: 10px 4px !important; font-size: 14px !important; font-weight: 600 !important; color: ${isDark ? "#98989d" : "#6e6e73"} !important; cursor: pointer !important; user-select: none !important; }
    `;
    ...
    Object.assign(ytproDownDiv.style, {
        ...
        padding: "14px 18px 24px", borderRadius: "32px 32px 0 0",
    ...
    Object.assign(tabs.style, {
        ...
        background: isDark ? "#212124" : "#f6f6f9",
        borderRadius: "16px", padding: "4px", gap: "4px",
        marginBottom: "14px", border: `1px solid ${isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)"}`
    });
    ...
    TABS.forEach(({ label, viewId }, idx) => {
        ...
        tab.style.cssText = `flex:1;display:flex;align-items:center;justify-content:center;border-radius:12px;font-size:13.5px;font-weight:600;cursor:pointer;transition:all 0.2s ease;color:${isDark ? "#8e8e93" : "#86868b"};`;
    ...
    tabs.children[0].style.background = isDark ? "rgba(255,255,255,0.12)" : "#ffffff";
    tabs.children[0].style.color = isDark ? "#ffffff" : "#000000";
    ...
    tabs.addEventListener("click", (e) => {
        ...
        [...tabs.children].forEach(child => {
            ...
            child.style.color = isDark ? "#8e8e93" : "#86868b";
        ...
        el.style.background = isDark ? "rgba(255,255,255,0.12)" : "#ffffff";
        el.style.color = isDark ? "#ffffff" : "#000000";
        el.style.boxShadow = "0 2px 8px rgba(0,0,0,0.1)";
    });

    // ── Bottom Action Row: Clear Data & Close ──
    const bottomBar = document.createElement("div");
    bottomBar.style.cssText = `display:flex;align-items:center;justify-content:space-between;margin-top:16px;padding-top:14px;border-top:1px solid ${isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.05)"};`;

    const clearBtn = document.createElement("button");
    clearBtn.style.cssText = `background:rgba(255,59,48,0.12);color:#ff3b30;border:none;border-radius:14px;padding:9px 16px;font-size:13px;font-weight:600;display:flex;align-items:center;gap:6px;cursor:pointer;`;
    clearBtn.innerHTML = `
        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="#ff3b30" viewBox="0 0 16 16"><path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/><path fill-rule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z"/></svg>
        <span>Clear Data</span>
    `;

    clearBtn.addEventListener("click", () => {
        window._ytproCache = null;
        window._ytInstance = null;
        for (let key in pendingStreams) delete pendingStreams[key];
        document.getElementById("ytproDownloadIndicator")?.remove();
        document.getElementById("ytProDownloaderDiv")?.remove();
        document.getElementById("outerdownytprodiv")?.remove();
        window.Android?.showToast?.("Download cache & temporary data cleared!");
    });

    const closeBtn = document.createElement("button");
    closeBtn.style.cssText = `background:${isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)"};color:${themeColor};border:none;border-radius:14px;padding:9px 18px;font-size:13px;font-weight:600;cursor:pointer;`;
    ...
    ytproDownDiv.appendChild(bottomBar);
...
async function pipeToDiskFast(stream, fileName, expectedTotalBytesStr, elDetails, elProgress) {
    ...
    try {
        const CHUNK_SIZE = 1024 * 1024; // 1MB Buffer for High-Speed Streams
        while (true) {
...
function createDownloaderStatus() {
    ...
    Object.assign(div.style, {
        ...
        display: "none", background: isDark ? "#1e1e20" : "#ffffff",
